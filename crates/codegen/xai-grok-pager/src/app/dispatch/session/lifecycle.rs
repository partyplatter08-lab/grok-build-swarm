...
pub(in crate::app::dispatch) fn handle_switch_model_complete(
    ...
    effort_option_id: Option<String>,
    ...
    if let Some(agent) = app.agents.get_mut(&agent_id) {
        ...
        let mut effects = match result {
            Ok(()) => {
                ...
                let prev_option = agent.session.models.reasoning_effort_option_id.clone();
                agent.session.models.set_current_with_option(
                    model_id.clone(),
                    effort,
                    effort_option_id.clone(),
                );
                let resolved_effort = agent.session.models.reasoning_effort;
                let resolved_option = agent.session.models.reasoning_effort_option_id.clone();
