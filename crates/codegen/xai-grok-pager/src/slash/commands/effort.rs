...
impl SlashCommand for EffortCommand {
    ...
    fn suggest_args(&self, ctx: &AppCtx, _args_query: &str) -> Option<Vec<ArgItem>> {
        ...
        Some(crate::slash::commands::effort_levels::build_effort_arg_items_with_option_id(
            &options,
            ctx.models.reasoning_effort,
            ctx.models.reasoning_effort_option_id.as_deref(),
            true,
    ...
    fn run(&self, ctx: &mut CommandExecCtx, args: &str) -> CommandResult {
        ...
        match ctx.models.resolve_effort_for_model(&model_id, trimmed) {
            Ok(effort) => {
                let option_id = ctx
                    .models
                    .resolve_effort_token_with_id_for(&model_id, trimmed)
                    .map(|(_, id)| id)
                    .unwrap_or_else(|| trimmed.to_ascii_lowercase());
                CommandResult::Action(Action::SwitchModel {
                    model_id,
                    effort: Some(effort),
                    effort_option_id: Some(option_id),
                })
            }
...
mod tests {
    ...
    fn valid_level_dispatches_switch_model_on_current() {
        ...
        match result {
            CommandResult::Action(Action::SwitchModel { model_id, effort, effort_option_id: _ }) => {
    ...
    fn suggest_args_lists_levels_with_active_marker() {
        ...
        // Multi-agent modes (3) + classic single-agent levels (4).
        assert_eq!(items.len(), 7);
        assert_eq!(items[0].insert_text, "swarm-heavy");
        assert_eq!(items[1].insert_text, "swarm");
        assert_eq!(items[2].insert_text, "heavy");
        assert_eq!(items[3].insert_text, "xhigh");
        assert_eq!(items[4].insert_text, "high");
        assert_eq!(items[4].display, "high (active)");
        assert_eq!(items[5].insert_text, "medium");
        assert_eq!(items[6].insert_text, "low");
        assert!(items[0].match_text.starts_with("a "));
        assert!(items[6].match_text.starts_with("g "));
    }
}
